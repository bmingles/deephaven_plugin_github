...
from .deephaven_plugin_github_object import DeephavenPluginGithubObject

# Create a custom message stream to send messages to the client

class DeephavenPluginGithubMessageStream(MessageStream):
    """
...
# The value returned by name() should match supportedTypes in DeephavenPluginGithubPlugin.ts
class DeephavenPluginGithubType(BidirectionalObjectType):
    """
    ...
    @property
    def name(self) -> str:
        return "DeephavenPluginGithub"

    def is_type(self, obj: Any) -> bool:
        return isinstance(obj, DeephavenPluginGithubObject)

    def create_client_connection(
        self, obj: object, connection: MessageStream
    ) -> MessageStream:
        message_stream = DeephavenPluginGithubMessageStream(obj, connection)
