...
from .js_plugin import DeephavenPluginGithubJsPlugin
from .deephaven_plugin_github_type import DeephavenPluginGithubType

# The namespace that the Python plugin will be registered under.
PACKAGE_NAMESPACE = "deephaven_plugin_github"
# Where the Javascript plugin is. This is set in setup.py.
JS_NAME = "_js"
# The JsPlugin class that will be created and registered.
PLUGIN_CLASS = DeephavenPluginGithubJsPlugin


class DeephavenPluginGithubRegistration(Registration):
    @classmethod
    def register_into(cls, callback: Callback) -> None:

        # Register the Python plugin
        callback.register(DeephavenPluginGithubType)
